...
const RequestComponent = ({ template }: Props) => {

    let request: IRequest = template
    ...
    return (
        <form action="/api/token/request/" method="post">
            ...
            <input type="datetime-local" id="timestamp" name="timestamp" className={styles.input} value={new Date(request.timestamp).toISOString().slice(0, 16)} readOnly />
